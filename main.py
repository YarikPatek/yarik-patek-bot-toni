...
async def start_msg(message: types.Message):
    ...
    await bot.send_message(message.chat.id,
                           f'Здраствуй, {message.chat.first_name}, Я футбольный бот и я умею показывать актуальную стоимость и другую полезную информацию о каждом футболисте, попробуй сам!\nИнструкция:'
                           f'\nНажмите кнопку /Поиск_футболиста или введите вручную чтобы начать поиск игрока, введите полное или хотя бы частичное имя или фамилию футболиста'
                           f', например:"Cristiano Ronaldo, Pedri, Foden", если поиск не дал результатов, то попробуйте заменить язык вввода, '
                           f'например:"Вместо Роналдо напишите Ronaldo", также не забывайте что фамилии русских игроков лучше писать русскими буквами, а фамилии иностранных - латинскими буквами'
                           f'\nНадеюсь бот будет вам полезен, Удачи!'
                           f''
                           ,
...
async def send_random_value(call: types.CallbackQuery):
    ...
    url_receiving = soup.find('div', class_='dataBild').find('img')
    get_url = url_receiving.get('src')
    img = requests.get(get_url)
    img_open = open('Player3.jpg', 'wb')
    img_open.write(img.content)
    img_open.close()
    background = requests.get('https://thumb.tildacdn.com/tild6536-3238-4531-a131-353263313330/-/resize/824x/-/format/webp/7.jpg')
